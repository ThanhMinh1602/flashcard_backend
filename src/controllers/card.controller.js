...
import { isImageDataUrl, sanitizeCanvasData } from '../utils/canvasData.js';
import {
...
function mergeSidePayload(existingSide = {}, payload = {}) {
  return {
    ...(existingSide || {}),
    ...(payload || {}),
  };
...
function cleanPayload(body = {}) {
  ...
  delete payload.__v;
  delete payload.undoStack;
  delete payload.redoStack;
...
export const saveCardSide = asyncHandler(async (req, res) => {
  ...
  if (side === 'back') {
    updateData.back = mergeSidePayload(existingCard?.back, payload);
  } else {
    updateData.front = mergeSidePayload(existingCard?.front, payload);
  }

  const nextFront = side === 'front' ? updateData.front : existingCard?.front;
  const nextBack = side === 'back' ? updateData.back : existingCard?.back;
...
export const bulkSaveCards = asyncHandler(async (req, res) => {
  ...
  const operations = cardsToSave
    .map((card, index) => {
      ...
      const existingCard = existingCardMap.get(localId);
      const frontPayload = cleanPayload(card.front || {});
      const backPayload = cleanPayload(card.back || {});
      const front = mergeSidePayload(existingCard?.front, frontPayload);
      const back = mergeSidePayload(existingCard?.back, backPayload);
